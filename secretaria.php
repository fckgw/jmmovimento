 ...
 * Versão: 100% Completa | Blindagem Total de Bibliotecas e Filtros
 */
error_reporting(E_ALL); 
ini_set('display_errors', 1);
require_once 'config.php';

// --- CARREGAMENTO BLINDADO DA BIBLIOTECA EXCEL ---
$lib_carregada = false;
if (file_exists('SimpleXLSX.php')) { 
    require_once 'SimpleXLSX.php';
    // Verifica qual classe está disponível (versão nova com namespace ou versão antiga global)
    if (class_exists('\Shuchkin\SimpleXLSX')) {
        $classe_xlsx = '\Shuchkin\SimpleXLSX';
        $lib_carregada = true;
    } elseif (class_exists('SimpleXLSX')) {
        $classe_xlsx = 'SimpleXLSX';
        $lib_carregada = true;
    }
}
...
// Sincronização de Nível em Tempo Real
try {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if ($acao == 'novo_projeto') {
            ...
            if (!empty($id_p)) {
                $pdo->prepare("UPDATE projetos SET nome_projeto=?, data_inicio=?, data_fim=?, mensagem=?, tem_anexo=?, anexo_path=? WHERE id=?")
                    ->execute([$nome_p, $ini, $fim, $msg_txt, $tem_anexo, $anexo_path, $id_p]);
            } else {
                $pdo->prepare("INSERT INTO projetos (nome_projeto, data_inicio, data_fim, mensagem, tem_anexo, anexo_path) VALUES (?,?,?,?,?,?)")
                    ->execute([$nome_p, $ini, $fim, $msg_txt, $tem_anexo, $anexo_path]);
            }
        ...
        if ($acao == 'deletar_projeto') {
            ...
            header("Location: secretaria.php?tab=projetos&msg_del=1"); exit;
        }

        // 3. IMPORTAR EXCEL (BLINDADO)
        if ($acao == 'importar_marketing') {
            if (!$lib_carregada) {
                header("Location: secretaria.php?tab=marketing&erro_lib=1"); exit;
            }
                ...
                if ($xlsx = $classe_xlsx::parse($_FILES['arquivo_excel']['tmp_name'])) {
                    ...
                    foreach ($xlsx->rows() as $i => $row) {
                        ...
                        if (!empty($nome) && !empty($tel)) {
                            if (in_array($tel, $tels_no_banco)) { $dups++; }
                            else {
                    ...
                    header("Location: secretaria.php?tab=marketing&f_projeto=$proj_id&n=$novos&d=$dups"); exit;
                }
            }
        }

        // 4. SALVAR FIEL / DOAÇÃO
        if ($acao == 'salvar_fiel') {
            ...
            header("Location: secretaria.php?tab=fieis&fiel_ok=1&f_projeto=$proj_id"); exit;
        }

        if ($acao == 'deletar_fiel') {
            $pdo->prepare("DELETE FROM rifas_pagamentos WHERE id = ?")->execute([$_POST['id_f']]);
            header("Location: secretaria.php?tab=fieis&del_fiel_ok=1"); exit;
        }
...
<head>
    ...
    <title>Secretaria JMM Master</title>
...
<nav class="navbar navbar-light bg-white shadow-sm mb-3 sticky-top">
    <div class="container d-flex justify-content-between align-items-center">
        ...
        <h6 class="m-0 fw-bold text-uppercase">Secretaria / Gestão</h6>
        ...
        <div class="tab-pane fade show active" id="tab-projetos">
            <div class="card p-4 border-top border-5 border-primary shadow">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="anexo_atual" id="anexo_atual">
                    <input type="text" name="nome_p" id="f_nome" class="form-control mb-2 shadow-sm" placeholder="Nome do Projeto" required>
                    ...
                    <div class="card p-2 bg-light mb-2 border-0">
                        <div class="form-check form-switch"><input class="form-check-input" type="checkbox" name="tem_anexo" id="f_tem_anexo" onchange="toggleAnexo(this.checked)"><label class="form-check-label fw-bold small">Possui imagem em anexo?</label></div>
            ...
            <div class="table-responsive bg-white rounded-4 shadow-sm">
                <table class="table align-middle mb-0 table-custom">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-3 fw-bold text-uppercase"><?=$p['nome_projeto']?> <?=$p['tem_anexo']?'<i class="bi bi-image text-primary"></i>':''?></td>
                            ...
                            <td class="text-end pe-3 text-nowrap">
                                <button type="button" class="btn btn-link text-primary p-0 me-3 btn-edit-proj" 
                                    ...
                                    data-msg="<?=htmlspecialchars($p['mensagem'])?>" data-anexo="<?=$p['tem_anexo']?>" data-path="<?=$p['anexo_path']?>">
        ...
        <div class="tab-pane fade" id="tab-marketing">
            ...
            <?php if(!empty($f_proj) && $proj_mkt_ativo): ?>
                <div class="card p-4 border-top border-5 border-success text-center shadow-sm mb-3">
                    <h6 class="fw-bold mb-3">Importar Excel para: <u><?=$proj_mkt_ativo['nome_projeto']?></u></h6>
                    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="tab-pane fade" id="tab-fieis">
            ...
            <div class="filter-bar">
                <form method="GET" class="row g-2">
                    ...
                    <div class="col-md-3"><select name="f_projeto" class="form-select form-select-sm"><option value="">-- Projetos --</option><?php foreach($projetos_select as $p): ?><option value="<?=$p['id']?>" <?=($f_proj == $p['id'] ? 'selected':'')?>><?=$p['nome_projeto']?></option><?php endforeach; ?></select></div>
                    <div class="col-6 col-md-2"><input type="text" name="f_data_ini" class="form-control form-control-sm" placeholder="De" onkeyup="maskData(this)" value="<?=$f_ini?>"></div>
                    <div class="col-6 col-md-2"><input type="text" name="f_data_fim" class="form-control form-control-sm" placeholder="Até" onkeyup="maskData(this)" value="<?=$f_fim?>"></div>
                    <div class="col-8 col-md-3"><input type="text" name="f_cidade" class="form-control form-control-sm" placeholder="Cidade" value="<?=$f_cidade?>"></div>
                    <div class="col-4 col-md-2"><button type="submit" class="btn btn-primary btn-sm w-100 fw-bold">FILTRAR</button></div>
                </form>
            </div>
            ...
            <div class="collapse" id="formFiel">
                <div class="card p-4 border-top border-5 border-info shadow">
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row g-2 mb-2">
                            <div class="col-8"><label class="small fw-bold">PROJETO</label><select name="projeto_id" id="f_proj_id" class="form-select"><?php foreach($projetos_select as $p): ?><option value="<?=$p['id']?>"><?=$p['nome_projeto']?></option><?php endforeach; ?></select></div>
                        ...
                        <div class="row g-2 mb-3">
                            <div class="col-8"><input type="text" name="telefone" id="f_tel" class="form-control" placeholder="Telefone"></div>
                            <div class="col-4"><input type="text" name="data_pagamento" id="f_data_p" class="form-control" placeholder="DD/MM/AAAA" onkeyup="maskData(this)"></div>
                        </div>
                        <div class="row g-2 mb-3">
                        ...
                        <input type="file" name="comprovante" class="form-control mb-3">
                        <button type="submit" id="btn_fiel" class="btn btn-info w-100 fw-bold text-white shadow">SALVAR</button>
                    </form>
                </div>
            </div>
            <div class="table-responsive bg-white rounded-4 shadow-sm">
                <table class="table align-middle mb-0 table-custom">
                    <thead class="table-dark"><tr><th>Data</th><th>Fiel</th><th>Rifa</th><th>Valor</th><th class="text-end">Ação</th></tr></thead>
                    <tbody>
                        <?php foreach($lista_fieis as $lf): ?>
                        <tr><td><?=dataBR($lf['data_pagamento'])?></td>
                        ...
                        <td class="text-end text-nowrap">
                            ...
                            <button class="btn btn-link text-danger p-0" onclick="confirmaDelFiel(<?=$lf['id']?>, '<?=$lf['numero_rifa']?>', '<?=$lf['nome_fiel']?>')"><i class="bi bi-trash fs-5"></i></button>
                        </td></tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <div class="tab-pane fade" id="tab-relatorios">
            <div class="card p-5 border-top border-5 border-dark text-center shadow">
                ...
                <form action="gerar_relatorio_pdf.php" method="GET" target="_blank" class="mb-4">
                    ...
                    <button type="submit" class="btn btn-dark btn-lg w-100 fw-bold">GERAR PDF</button>
                </form>
...
<div class="modal fade" id="modalVerFoto" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header border-0 pb-0"><h6 class="fw-bold m-0">Comprovante</h6><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body text-center p-4"><img src="" id="imgPreview" class="img-fluid rounded shadow"></div></div></div></div>
...
<script>
    ...
    function confirmaDelFiel(id, rifa, fiel) {
        Swal.fire({ title: 'Excluir Registro?', html: `Rifa Nº: <b>${rifa}</b><br>Fiel: <b>${fiel}</b>`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: 'DELETAR'
    ...
    function editarFiel(f) {
        ...
        document.getElementById('f_cid').value = f.cidade; document.getElementById('f_est').value = f.estado; document.getElementById('comp_atual').value = f.comprovante_path;
    ...
    document.querySelectorAll('.btn-assistente').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            navigator.clipboard.writeText(msg).then(() => {
                Swal.fire({ icon: 'success', title: 'Copiado!', text: 'Mensagem pronta para colar no WhatsApp.', confirmButtonText: 'ABRIR WHATSAPP', confirmButtonColor: '#25d366'
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if(p.get('erro_lib')) Swal.fire('Falta um arquivo!', 'O arquivo SimpleXLSX.php nao foi encontrado.', 'error');
    });
</script>
</body>
</html>