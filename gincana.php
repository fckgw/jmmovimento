 ...
 * Módulos: Chamada, Cabo de Guerra, Prova, Jovens (com Edit), Ata e Ranking.
 */
...
// 2. DECLARAÇÃO DE VARIÁVEIS PARA EVITAR WARNINGS
$msg = ''; $erro = '';
$encontros_all = []; $enc_ativo = null; $jovens_grid = []; $ranking_gincana = [];
$cg_times = []; $disputas_cg = []; $jovens_presentes = []; $aniv_hoje = [];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($acao == 'checkin_massa') {
        if (!empty($_POST['jovens_ids']) && isset($_POST['e_id_ativo'])) {
            foreach ($_POST['jovens_ids'] as $jid) {
                $pdo->prepare("INSERT IGNORE INTO presencas (jovem_id, encontro_id, metodo) VALUES (?, ?, 'manual')")->execute([$jid, $_POST['e_id_ativo']]);
            }
        }
    }
    ...
    // GESTÃO DE JOVENS (INSERT OU UPDATE)
    if ($acao == 'novo_jovem') {
        ...
        if (!empty($id_j)) {
            // UPDATE
            $stmt = $pdo->prepare("UPDATE jovens SET nome=?, telefone=?, ano_nascimento=?, data_nascimento=? WHERE id=?");
            $stmt->execute([$nome, $tel, $ano, $data_nasc, $id_j]);
            registrarLog($pdo, "Atualizou dados do jovem: $nome", "Jovens");
        } else {
            // INSERT
            ...
            registrarLog($pdo, "Cadastrou novo jovem: $nome", "Jovens");
        }
    }
    if ($acao == 'deletar_jovem') {
        $pdo->prepare("DELETE FROM jovens WHERE id = ?")->execute([$_POST['id_jovem']]);
        registrarLog($pdo, "Excluiu jovem ID: " . $_POST['id_jovem'], "Jovens");
    }
    ...
    header("Location: gincana.php?tab=$aba_retorno"); exit;
}
...
$f_j = $_GET['f_jovem'] ?? '';
$where_j = $f_j ? "WHERE nome LIKE '%$f_j%' OR telefone LIKE '%$f_j%'" : "";
$jovens_grid = $pdo->query("SELECT * FROM jovens $where_j ORDER BY nome ASC LIMIT $offset, $itens_por_pag")->fetchAll(PDO::FETCH_ASSOC);
$total_paginas = ceil(($f_j ? count($jovens_grid) : $total_cadastrados) / $itens_por_pag);
...
<head>
    ...
    <style>
        ...
        .aniv-box { background: #fff3cd; border-left: 5px solid #ffc107; padding: 10px; border-radius: 10px; margin-bottom: 10px; }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <?php if($aniv_hoje): ?>
    <div class="aniv-box shadow-sm small">
        <h6 class="fw-bold m-0"><i class="bi bi-cake2-fill text-danger"></i> ANIVERSÁRIO HOJE!</h6>
        <?php foreach($aniv_hoje as $ah) echo $ah['nome'] . " 🎈 "; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane fade" id="tab-cg" role="tabpanel">
            ...
            <?php foreach($disputas_cg as $dp): ?>
                <div class="card p-3 text-center border-start border-5 border-primary">
                    <div class="d-flex justify-content-around mb-1"><div class="fw-bold"><?=$dp['nome_a']?></div><div class="small">VS</div><div class="fw-bold"><?=$dp['nome_b']?></div></div>
            ...
            <div class="card p-3 shadow-sm"><div class="d-flex justify-content-between mb-2"><h6>RANKING CABO</h6><a href="gerar_cg_pdf.php" target="_blank" class="btn btn-sm btn-danger py-0">PDF</a></div>
                <table class="table table-sm small"><tbody><?php foreach($cg_times as $idx => $ct): ?><tr><td><?=($idx+1)?>º</td><td class="fw-bold"><?=$ct['nome_santo']?></td><td><small><?=$ct['capitao_nome']?></small></td><td class="text-end"><b><?=$ct['pontos']?></b></td><td class="text-end"><button class="btn btn-link btn-sm p-0 text-primary me-2" onclick="editarTimeCG(<?=$ct['id']?>, '<?=$ct['nome_santo']?>', '<?=$ct['capitao_id']?>')"><i class="bi bi-pencil"></i></button><form method="POST" class="d-inline"><input type="hidden" name="form_acao" value="excluir_time_cg"><input type="hidden" name="time_id" value="<?=$ct['id']?>"><input type="hidden" name="aba_destino" value="cg"><button type="submit" class="btn btn-link btn-sm p-0 text-danger" onclick="return confirm('Excluir?')"><i class="bi bi-trash"></i></button></form></td></tr><?php endforeach; ?></tbody></table>
            </div>
        </div>
        ...
        <div class="tab-pane fade" id="tab-jovens" role="tabpanel">
            <div class="card p-3 border-top border-5 border-info shadow-sm">
                <h6 class="fw-bold mb-3" id="titulo_form_jovem">Novo Cadastro de Jovem</h6>
                <form method="POST" id="form_jovem_base">
                    ...
                    <input type="text" name="telefone" id="edit_tel" class="form-control mb-2" placeholder="WhatsApp">
                    
                    <div class="d-flex gap-2">
                        <button type="submit" id="btn_save_jovem" class="btn btn-info w-100 fw-bold text-white py-2">SALVAR CADASTRO</button>
                        <button type="button" id="btn_cancel_edit" class="btn btn-light border d-none" onclick="cancelarEdicaoJovem()">CANCELAR</button>
                    </div>
                </form>
            </div>

            <form method="GET" class="d-flex gap-1 mb-3"><input type="hidden" name="tab" value="jovens"><input type="text" name="f_jovem" class="form-control form-control-sm" placeholder="Buscar 76 jovens..." value="<?=htmlspecialchars($f_j)?>"><button type="submit" class="btn btn-dark btn-sm"><i class="bi bi-search"></i></button></form>

            <div class="table-responsive"><table class="table table-sm bg-white rounded border"><tbody><?php foreach($jovens_grid as $jg): ?>
                <tr class="align-middle"><td class="ps-3"><div class="fw-bold small"><?=$jg['nome']?></div><small class="text-muted small"><?=$jg['telefone']?> | <?=($jg['data_nascimento']?date('d/m/y',strtotime($jg['data_nascimento'])):$jg['ano_nascimento'])?></small></td><td class="text-end pe-3">
                    <button class="btn btn-link text-primary p-0 me-2" onclick="povoarEdicaoJovem(<?=htmlspecialchars(json_encode($jg))?>)"><i class="bi bi-pencil-square"></i></button>
                    <form method="POST" class="d-inline"><input type="hidden" name="form_acao" value="deletar_jovem"><input type="hidden" name="id_jovem" value="<?=$jg['id']?>"><button type="submit" class="btn btn-link text-danger p-0" onclick="return confirm('Excluir?')"><i class="bi bi-trash"></i></button></form>
                </td></tr>
...
<div class="modal fade" id="modalEditCg" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><form method="POST" class="modal-content border-0 shadow">
    ...
    <div class="modal-header bg-warning"><h5>Editar Time</h5></div><div class="modal-body p-4">
    ...
    </div><div class="modal-footer"><button type="submit" class="btn btn-warning w-100 fw-bold">SALVAR</button></div>
</form></div></div>
...
<div class="modal fade" id="modalMassa" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><form method="POST" class="modal-content border-0 shadow-lg"><input type="hidden" name="form_acao" value="checkin_massa"><input type="hidden" name="aba_destino" value="chamada"><input type="hidden" name="e_id_ativo" value="<?=$enc_id_ativo?>">
...
<script>
    ...
    function povoarEdicaoJovem(j) {
        ...
        document.getElementById('titulo_form_jovem').innerText = "Editar Jovem: " + j.nome;
    ...
    function cancelarEdicaoJovem() {
        document.getElementById('form_jovem_base').reset();
        document.getElementById('id_jovem_edit').value = "";
        document.getElementById('titulo_form_jovem').innerText = "Novo Cadastro de Jovem";
        document.getElementById('btn_save_jovem').innerText = "SALVAR CADASTRO";
        document.getElementById('btn_save_jovem').classList.replace('btn-warning', 'btn-info');
    ...
    function buscarChamada(q) {
        ...
        fetch('acoes_presenca.php?buscar=' + q).then(r => r.json()).then(dados => {
            let h = ''; dados.forEach(d => { h += `<button type="button" onclick="salManual(${d.id})" class="list-group-item small fw-bold py-2 text-uppercase">${d.nome}</button>`; });
            res.innerHTML = h;
        });
    }
    function salManual(jId) { const fd = new FormData(); fd.append('jovem_id', jId); fd.append('encontro_id', <?=$enc_id_ativo?>); fetch('acoes_presenca.php?salvar_manual=1', { method: 'POST', body: fd }).then(() => location.reload()); }
    function toggleMassa() { document.querySelectorAll('.check-m').forEach(c => c.checked = !c.checked); }

    // --- CABO DE GUERRA ---
    function editarTimeCG(id, nome, cap) {
        document.getElementById('edit_cg_id').value = id;
        document.getElementById('edit_cg_nome').value = nome;
        document.getElementById('edit_cg_cap').value = cap;
        new bootstrap.Modal(document.getElementById('modalEditCg')).show();
    }
    let cgTimers = {}, cgInts = {};
    ...
    function vencerCg(dId, idA, idB, nA, nB) {
        ...
        document.getElementById('modal_id_disputa').value = dId; document.getElementById('modal_tempo_cg').value = t;
        document.getElementById('btn_time_a').innerText = nA; document.getElementById('btn_time_a').value = idA;
        document.getElementById('btn_time_b').innerText = nB; document.getElementById('btn_time_b').value = idB;
